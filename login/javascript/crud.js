...
function searchUser(searchTerm) {
    ...
    // تصفية المستخدمين المتطابقين
    Usercontainer.forEach((user, index) => {
        if (user.name.toLowerCase().includes(lowerCaseSearchTerm) || 
            ...
            cartona += `<tr>
                ...
                <td>${user.phone}</td>
                <td>${user.sessions}</td>
                <td><button class="btn btn-outline-warning" onclick="Update(${index})">Update</button></td>
                <td><button class="btn btn-outline-danger" onclick="delete_user(${index})">Delete</button></td>
                </tr>`;
        }
    ...
    // تحديث محتوى الجدول
    if (cartona === '') {
        cartona = `<tr><td colspan="7">لا توجد نتائج</td></tr>`;
    }
